...
export function buildAxolotlConfig(ctx: RenderContext, dataFiles: SplitFileMap): string {
  ...
  const validationPath =
    dataFiles.validation !== undefined ? `./${dataFiles.validation}` : undefined;
  ...
  const header = [
    ...
    ...(type === 'rl'
      ? [
          '# NOTE: GRPO requires reward functions — see the Axolotl RL docs and add',
          '# a `trl.reward_funcs` section before training.',
        ]
      : []),
    '',
  ].join('\n');
