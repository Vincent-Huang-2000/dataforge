...
function Stat({ label, value }: { label: string; value: number }) {
  return (
    <div className="border-hairline bg-surface-2 rounded-(--radius-control) border px-2 py-1.5">
      <div className="tech-label">{label}</div>
      <div className="text-ink font-mono text-sm tabular-nums">{fmtNum(value)}</div>
    </div>
  );
}
...
export function BackupSection() {
  ...
  return (
    <>
      <div className="panel divide-hairline divide-y">
        <div className="flex items-center justify-between gap-3 px-3 py-2.5">
          <div className="min-w-0">
            <div className="text-ink text-sm">Full backup</div>
            <p className="text-ink-dim text-[13px]">
              Projects, examples, provider settings and app settings in one JSON file. API keys and
        ...
        <div className="flex items-center justify-between gap-3 px-3 py-2.5">
          <div className="min-w-0">
            <div className="text-ink text-sm">Restore</div>
            <p className="text-ink-dim text-[13px]">
              Merge a backup file into this browser's data. Rows with matching ids are overwritten.
            </p>
          </div>
      ...
      >
        {pending && (
          <DialogContent title="Restore backup" description="Merges into your current data by id.">
            <p className="text-ink-dim text-[13px] leading-relaxed">
              Nothing is deleted, but existing projects, examples, provider settings and app
              settings with matching ids will be overwritten by the backup contents.
            </p>
...
export function DangerZone() {
  ...
  return (
    <>
      <div className="border-danger/40 bg-danger/5 flex items-center justify-between gap-3 rounded-(--radius-panel) border px-3 py-2.5">
        <div className="min-w-0">
          <div className="text-ink text-sm font-medium">Delete all data</div>
          <p className="text-ink-dim text-[13px]">
            Wipes every project, example, job, provider config and cached response from this
      ...
      >
        <DialogContent title="Delete all data" description="Permanently erases the local database.">
          <p className="text-ink-dim text-[13px] leading-relaxed">
            Everything DataForge has stored in this browser will be erased and the app will reload
            empty. There is no undo and no recovery. Download a backup first if you might need this
            data again.
          </p>
          <div className="mt-3">
